...
<table width="350" cellpadding="3" cellspacing="3" style="border: 1px solid">
	...
	<?php
		...
		while($row = mssql_fetch_array($column_query))
		{
			...
			echo '<tr>';
			echo('<td align="center" style="background: ' . $bg . '"><input type="checkbox" name="columns[]" value="' . $row['COLUMN_NAME'] . '"></td>');
			...
			echo('<td align="center" style="background: ' . $bg . '">' . $row['IS_NULLABLE'] . '</td>');
			echo('<td align="center" style="background: ' . $bg . '"><a href="table_modify.php?mode=change&table=' . $_GET['table'] . '&column=' . urlencode($row['COLUMN_NAME']) . '&step=1">Change</a></td>');
			echo('<td align="center" style="background: ' . $bg . '"><a href="table_modify.php?mode=drop&table=' . $_GET['table'] . '&column=' . urlencode($row['COLUMN_NAME']) . '">Drop</a></td>');
			echo '</tr>';
	...
	<tr>
		<td align="center" colspan="3" style="background: #D0DCE0">
			<a href="table_insert.php?table=<?php echo(urlencode($_GET['table'])); ?>">Insert A Row</a>
		</td>
		<td align="center" colspan="4" style="background: #D0DCE0">
			<a href="table_modify.php?mode=add&table=<?php echo(urlencode($_GET['table'])); ?>&step=1">Create A New Column</a>
		</td>
	</tr>
</table>
